...
oauth = OAuth1Session(api_key, client_secret=client_secret) #ceate an oauth section, pass in client key and client secret
fetch_response = oauth.fetch_request_token(request_token_url)
resource_owner_key = fetch_response.get('oauth_token') #library goes and does under cover stuff, gets response back, from response get Oauth token and oauth token secret
resource_owner_secret = fetch_response.get('oauth_token_secret') #This is our request token!
